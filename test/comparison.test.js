...
const fs = require('fs');
const comp = require('../lib/comparison');
const Comparison = comp.Comparison;
const MethodReport = comp.MethodReport;
const BasicComparison = comp.BasicComparison;
...
test('correctly parse getter functions', t => {
  const content = fs.readFileSync('./test/fixtures/getter.js', 'utf-8');
  const doc = new AtomDocDocument(content);
  return doc
    .process()
    .then(result => {
      const comparer = new Comparison(result);
      t.true(comparer.valid);
    })
    .catch(console.log);
});
