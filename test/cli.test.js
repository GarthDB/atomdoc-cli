...
test.cb("should display help", (t) => {
  nixt()
    .expect((result) => {
      t.true(result.stdout.trim().indexOf("Usage: ") >= 0);
    })
...
test.cb("should generate correct output", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js --reporter false ./test/fixtures/basic_function.js")
    .end(t.end);
});
test.cb("should write json file", (t) => {
  nixt()
    ...
    .run(
      "node ./bin/index.js -o ./test/expected/tmp/api.json ./test/fixtures/has_comment.js"
    )
    .end(t.end);
});
test.cb("should error out if file doesn't exist", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js nonexistent.js")
    .end(t.end);
});
test.cb("should return code 1 when errors exist", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js ./test/fixtures/basic_reporter.js")
    .end(t.end);
});
test.cb("should return code 0 when no errors exist", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js ./test/fixtures/basic_function.js")
    .end(t.end);
});
test.cb("should return a cli verbose report", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js ./test/fixtures/basic_reporter.js --verbose")
    .end(t.end);
});
test.cb("should show a report when no docs present", (t) => {
  nixt()
    ...
    .run("node ./bin/index.js ./test/fixtures/no_docs.js --verbose")
    .end(t.end);
});
test.cb("should use a custom reporter", (t) => {
  nixt()
    ...
    .run(
      'node ./bin/index.js --reporter="./test/fixtures/console_reporter.js" ./test/fixtures/basic_function.js'
    )
    .end(t.end);
});
